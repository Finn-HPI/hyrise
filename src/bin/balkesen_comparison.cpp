...
[[maybe_unused]] void partition_phase(std::span<Relation> buckets_r, std::span<Relation> buckets_s,
  ...
  {
    auto radix_partition_r = RadixPartition2<ColumnType>(&chunk_r, &tmp_chunk_r, buckets_r, CLUSTER_COUNT);
    radix_partition_r.execute();
  }
  {
    auto radix_partition_s = RadixPartition2<ColumnType>(&chunk_s, &tmp_chunk_s, buckets_s, CLUSTER_COUNT);
...
[[maybe_unused]] void mwaymerge_phase(simd_vector<SimdElement>& merged_tuples_r,
  ...
  auto mway_merge_r = MultiwayMerger2<choose_count_per_vector(), SortingType>(parts_r, buffer_size);
  auto mway_merge_s = MultiwayMerger2<choose_count_per_vector(), SortingType>(parts_s, buffer_size);
...
int main(int argc [[maybe_unused]], char** argv) {
  if (argc < 4) {
    std::cerr << "Usage: " << argv[0] << " <s_multiplier> <num_threads> <L3_cache_size>\n";
    std::cerr << "Example: " << argv[0] << " 2 8 8388608\n";
    return EXIT_FAILURE;
  }
