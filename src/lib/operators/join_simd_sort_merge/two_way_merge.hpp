...
class TwoWayMerge<2, T> : public AbstractTwoWayMerge<2, T, TwoWayMerge<2, T>> {
 ...
 public:
  static void __attribute__((always_inline)) reverse(VecType& vec) {
  ...
  static void __attribute__((always_inline)) merge_base_size(VecType& input_a, VecType& input_b, VecType& out1,
                                                             VecType& out2) {
    // Level 1
...
class TwoWayMerge<4, T> : public AbstractTwoWayMerge<4, T, TwoWayMerge<4, T>> {
 ...
 public:
  ...
  static void __attribute__((always_inline)) merge_base_size(VecType& input_a, VecType& input_b, VecType& out1,
                                                             VecType& out2) {
    // Level 1
    auto lo1 = elementwise_min(input_a, input_b);
    auto hi1 = elementwise_max(input_a, input_b);
    auto lo1_perm = __builtin_shufflevector(lo1, hi1, LOWER_HALVES);
    auto hi1_perm = __builtin_shufflevector(lo1, hi1, UPPER_HALVES);
    // Level 2
    auto lo2 = elementwise_min(lo1_perm, hi1_perm);
    auto hi2 = elementwise_max(lo1_perm, hi1_perm);
    auto lo2_perm = __builtin_shufflevector(lo2, hi2, 0, 4, 2, 6);
    auto hi2_perm = __builtin_shufflevector(lo2, hi2, 1, 5, 3, 7);
    // Level 3
    auto lo3 = elementwise_min(lo2_perm, hi2_perm);
    auto hi3 = elementwise_max(lo2_perm, hi2_perm);

    auto lo3_perm = __builtin_shufflevector(lo3, hi3, 0, 4, 2, 6);
    auto hi3_perm = __builtin_shufflevector(lo3, hi3, 1, 5, 3, 7);
    out1 = __builtin_shufflevector(lo3_perm, hi3_perm, LOWER_HALVES);
    out2 = __builtin_shufflevector(lo3_perm, hi3_perm, UPPER_HALVES);
  }
};
...
class TwoWayMerge<8, T> : public AbstractTwoWayMerge<8, T, TwoWayMerge<8, T>> {
 ...
 public:
  ...
  static void __attribute__((always_inline)) merge_base_size(VecType& input_a, VecType& input_b, VecType& out1,
    ...
    // Level 4
    auto lo4 = elementwise_min(lo3_perm, hi3_perm);
    auto hi4 = elementwise_max(lo3_perm, hi3_perm);
