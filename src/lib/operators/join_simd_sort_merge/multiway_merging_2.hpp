...
class MultiwayMerger2 {
 ...
 public:
  ...
  explicit MultiwayMerger2(std::span<Relation*> sorted_buckets, size_t max_buffer_size)
  ...
  simd_sort::simd_vector<SimdElement> merge() {
    ...
    if (_leaf_count == TWO_REMAINING) {
      ...
      TwoWayMerge::template merge_variable_length<count_per_vector * 4>(
          left->template begin<T>(), right->template begin<T>(), reinterpret_cast<T*>(merged_output.data()),
          left->num_tuples, right->num_tuples);
 ...
 private:
  ...
  void _load_from_leaves() {
    ...
    static auto empty_bucket = Relation{.tuples = nullptr, .num_tuples = 0};
  ...
  size_t _load_and_merge_from_leaves(CircularBuffer& buffer, Relation& left_bucket, Relation& right_bucket) {
    ...
    auto merge_buckets_into_buffer = [&](std::span<SimdElement> output) {
      ...
      TwoWayMerge::template merge_multiway_merge_nodes<count_per_vector * 4>(
          reinterpret_cast<T*>(left_bucket.tuples), reinterpret_cast<T*>(right_bucket.tuples),
          reinterpret_cast<T*>(output.data()), count_reads_left, count_reads_right, count_writes,
          left_bucket.num_tuples, right_bucket.num_tuples, output.size());
    ...
    auto read_and_write_remaining = [&](Relation& bucket) {
      auto write_func = [&](std::span<SimdElement> output) {
        const auto number_of_writes = std::min(output.size(), bucket.num_tuples);
  ...
  size_t _leaf_count;
  std::span<Relation*> _sorted_buckets;
